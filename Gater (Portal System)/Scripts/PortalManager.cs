...
public class PortalManager : MonoBehaviour {
	...
	[Serializable] public class PortalFunctionalityClass {
		public GameObject[] ExcludedObjsFromWallTrigger = new GameObject[0];
	...
	void OnEnable () {
        ...
        for (int i = 0; i < PortalMaterials.Length; i++)
		...
		//Check if list of excluded objects from trigger have the collider component
		if (PortalFunctionality.ExcludedObjsFromWallTrigger.Length > 0)
			for (int j = 0; j < PortalFunctionality.ExcludedObjsFromWallTrigger.Length; j++)
				if (PortalFunctionality.ExcludedObjsFromWallTrigger[j])
					if (!PortalFunctionality.ExcludedObjsFromWallTrigger[j].GetComponent<Collider> ())
						Debug.LogError ("One excluded object doesn't have a collider component");

		#if UNITY_EDITOR
			EditorApplication.update = null;
				
			//Searching of existing required objects for teleport, and fill in the relative variables with it
			int PortalCamObjsSteps = 0;

			for (int k = 0; k < transform.GetComponentsInChildren<Transform> ().Length; k++) {
				if (transform.GetComponentsInChildren<Transform> () [k].name == "Camera " + PortalCamObjsSteps) {
					PortalCamObjs [PortalCamObjsSteps] = transform.GetComponentsInChildren<Transform> () [k].gameObject;

					PortalCamObjsSteps += 1;
				}

				if (transform.GetComponentsInChildren<Transform> () [k].name == "SceneviewRender")
			...
			if (!InGameCamera) {
				...
				if (CurrentProjectionResolution.x != ViewSettings.Projection.Resolution.x || CurrentProjectionResolution.y != ViewSettings.Projection.Resolution.y || ViewSettings.Projection.CurrentDepthQuality != ViewSettings.Projection.DepthQuality) {
					...
					for (int i = 0; i < RenderTexts.Length; i++) {
						if (RenderTexts [i]) {
							...
							RenderTexts [i].name = "RenderTexture " + i;
					...
					if (!SceneviewRender) {
						SceneviewRender = new GameObject ("SceneviewRender");
						...
						if (SceneviewRender.name != "SceneviewRender")
							SceneviewRender.name = "SceneviewRender";
				...
				for (int j = 0; j < PortalCamObjs.Length; j++) {
					if (j < ViewSettings.Recursion.Steps + 1) {
						if (!PortalCamObjs [j]) {
							PortalCamObjs [j] = new GameObject ("Camera " + j);
							...
							if (PortalCamObjs [j].name != "Camera " + j)
								PortalCamObjs [j].name = "Camera " + j;
							...
							if (RenderTexts [i]) {
								for (int j = ViewSettings.Recursion.Steps; j >= 0; j--) {
									if (PortalCamObjs [j]) {
										...
										PortalCamObjs [j].GetComponent<Camera> ().farClipPlane = 1000;
	...
	class InitMaterialsList { public Material[] Materials = new Material[0]; }
	...
	void OnTriggerEnter (Collider collision) {
        //Disable collision between player and objects excluded from trigger
        CheckExcludedTriggerList(collision.gameObject, true);
		...
		if (collision.gameObject != gameObject && !collision.GetComponent<PortalManager> () && !collision.name.Contains (collision.gameObject.GetHashCode ().ToString ()) && !collision.name.Contains ("Clone")) {
			...
			if (!collision.GetComponent<FirstPersonController> ()) {
					...
					if (PortalMesh && ProxDetCollidedObjs [i]) {
						if (CollidedObjs [i].transform.childCount > 0) {
							for (int j = 0; j < CollidedObjs [i].transform.GetComponentsInChildren<Transform> ().Length; j++)
								if (CollidedObjs [i].GetComponent<FirstPersonController> () && CollidedObjs [i].transform.GetComponentsInChildren<Transform> () [j].GetComponent<Camera> ())
							...
							if (!CloneCollidedObjs [i]) {
								...
								if (CloneCollidedObjs [i].transform.childCount > 0) {
									for (int k = 0; k < CloneCollidedObjs [i].transform.GetComponentsInChildren<Transform> ().Length; k++) {
										...
										if (CloneCollidedObjs [i].transform.GetComponent<FirstPersonController> () && CloneCollidedObjs [i].transform.GetComponentsInChildren<Transform> () [k].GetComponent<Camera> ())
								...
								if (CloneCollidedObjs [i].transform.childCount == 0 && CloneCollidedObjs[i].GetComponent<Camera>())
									ObjCloneCollidedCamObj [1] = CloneCollidedObjs [i];

								if (ObjCloneCollidedCamObj [0]) {
									CloneCollidedObjs [i].GetComponent<FirstPersonController> ().enabled = false;

									CloneCollidedObjs [i].GetComponent<FirstPersonController> ().m_OriginalCameraPosition = CollidedObjs [i].GetComponent<FirstPersonController> ().m_OriginalCameraPosition;
								}
	...
	void OnTriggerExit (Collider collision) {
        //Enable collision between player and objects excluded from trigger
        CheckExcludedTriggerList(collision.gameObject, false);
        ...
        for (int i = 0; i < CloneCollidedObjs.Length; i++) {
			if (ContinueTriggerEvents [i] && CollidedObjs [i] && CollidedObjs [i].GetHashCode ().ToString () == collision.gameObject.GetHashCode ().ToString () && CloneCollidedObjs [i]) {
				...
				GameObject[] CollidedObjsInternalParent = new GameObject[CollidedObjs [i].transform.childCount > 0 && !CollidedObjs [i].GetComponent<FirstPersonController> () ? CollidedObjs [i].transform.childCount : 0];
				...
				if (CollidedObjs [i].transform.childCount > 0) {
					for (int k = 0; k < CollidedObjs [i].transform.GetComponentsInChildren<Transform> ().Length; k++)
						if (CollidedObjs [i].GetComponent<FirstPersonController> () && CollidedObjs [i].transform.GetComponentsInChildren<Transform> () [k].GetComponent<Camera> ())
				...
				if (CloneCollidedObjs [i].transform.childCount > 0) {
					for (int k = 0; k < CloneCollidedObjs [i].transform.GetComponentsInChildren<Transform> ().Length; k++)
						if (CloneCollidedObjs [i].GetComponent<FirstPersonController> () && CloneCollidedObjs [i].transform.GetComponentsInChildren<Transform> () [k].GetComponent<Camera> ())
							ObjCloneCollidedCamObj [0] = CloneCollidedObjs [i].transform.GetComponentsInChildren<Transform> () [k].gameObject;
				}
				if (CloneCollidedObjs [i].transform.childCount == 0)
				...
				if (CollObjProxDetDistExit > 0) {
					if (!CollidedObjs [i].GetComponent<FirstPersonController> ()) {
						if (CollidedObjsInternalParent.Length == 0) {
                    ...
                    if (CollidedObjs[i].GetComponent<FirstPersonController>())
                    {
                        CollidedObjs[i].GetComponent<CharacterController>().enabled = false;
                        CollidedObjs[i].transform.position = CloneCollidedObjs[i].transform.position;
                        CollidedObjs[i].GetComponent<CharacterController>().enabled = true;

                        CollidedObjs[i].GetComponent<FirstPersonController>().m_MouseLook.Init(CollidedObjs[i].transform, CollidedObjs[i].GetComponent<FirstPersonController>().m_Camera.transform);
                    }
	...
	void CheckExcludedTriggerList (GameObject collision, bool IgnoreCollision) {
        if (PortalFunctionality.ExcludedObjsFromWallTrigger.Length > 0)
            for (int i = 0; i < PortalFunctionality.ExcludedObjsFromWallTrigger.Length; i++)
                if (PortalFunctionality.ExcludedObjsFromWallTrigger[i])
                    Physics.IgnoreCollision(PortalFunctionality.ExcludedObjsFromWallTrigger[i].GetComponent<Collider>(), collision.gameObject.GetComponent<Collider>(), IgnoreCollision);
    }
		...
		if (CollidedObjs.Length > 0) {
			if (!TriggerExit) {
				for (int i = 0; i < CollidedObjs.Length; i++) {
					...
					if (CollidedObjs [i] && CollidedObjs [i].transform.childCount > 0 && CollidedObjs [i].GetComponent<FirstPersonController> ()) {
